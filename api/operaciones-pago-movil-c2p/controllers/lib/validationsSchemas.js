...
const getBancosC2P = async() => {
    let arrayBancos = [];   
    if(typeof strapi.api["operaciones-pago-movil-c2p"]?.controllers['operaciones-pago-movil-c2p'].getBancosValidos == 'function'){
        const BancosValido =  await  strapi.api["operaciones-pago-movil-c2p"]?.controllers['operaciones-pago-movil-c2p'].getBancosValidos();
        if (BancosValido?.INFO?.body?.codigoRespuesta == "0000" && BancosValido?.INFO?.body?.bancos){
...
const CreateSchemeValidate = async() => {
    ...
    const getBancosArray = await getBancosC2P()
    ...
    return await yup.object().shape({
        ...
        datosCobros:yup.object().required("El campo 'datosCobros' es requerido").shape({
            telefonoCobrador: yup.string().required("El campo 'telefonoCobrador' es requerido").min(10, "El numero telefonico para el campo 'telefonoCobrador' deben ser de 10 caracteres exactos").max(10,"El numero telefonico para el campo 'telefonoCobrador' deben ser de 10 caracteres exactos").matches(strapi.config.get('sistemDefault.regexDefaultNumerosTelefonicos',""), { message: "Utilizo un nuemro telefonico no valido para el campo 'telefonoCobrador '. Debe contener 10 digitos numericos y ademas comenzar por el codigo  de la operadora (416) o regional (212) ej. 4164000939" , excludeEmptyString: true }),
            ...
            id: yup.string().required("El campo 'id' es requerido").min(12,"El campo 'id' debe poseer un tamaño de 12 caracteres").max(12, "El campo 'id' debe poseer un tamaño de 12 caracteres").matches(strapi.config.get('sistemDefault.regexDefaultDocumentoUsuario', /^$/),"El formato del campo 'id' no es valido, asegurese de que sigue el siguiente formato, Ej. V00023696753"),
            ...
            token: yup.string().required("El campo 'token' es requerido").min(6,"El campo 'token' debe poseer un tamaño de 6 caracteres").max(6, "El campo 'id' debe poseer un tamaño de 6 caracteres").matches(strapi.config.get('sistemDefault.regexDefaultToken', /^$/),"El formato del campo 'token' no es valido, asegurese de que posee solo digitos numerico y que sigue el siguiente formato, Ej. 546545"),
            ...
            origen: yup.string().required("El campo 'origen' es requerido").oneOf(strapi.config.get('sistemDefault.arrayOrigen',[]),"El campo 'origen' no es un valor valido")
          }),
