...
module.exports = {
    ...
    generateSignatureAndHeadersToken: () => {
        const nonce = (Date.now() * 1000).toString()
        let signature = `/${CONFIG.API_AUTH_TOKEN}${nonce}`
    ...
    insertarBasedeDatosSuccess: async(codigoApi, mensajeApi, referencia, codigoReverso, objetoDeConsulta, tasaBolivaresCobro, id_terminal, id_user, id_producto, id_sucursal, id_comercio , ip, storeDocumento, bancoOrigen, bancoDestino,  fecha, objectTasaComisiones, moneda_principal_transaccion,  MontoBolivares, MontoDolares, estatus, numero_de_documento_de_identificaion_fiscal, banco_aliado) => {
        const nombreTabla = CONFIG.MODEL_ID;
            ...
            documentoInsert.codigo_de_reverso = codigoReverso
            documentoInsert.nro_referencia_pagomovil = referencia
            documentoInsert.token_de_operacion = objetoDeConsulta?.token
            documentoInsert.banco_pagomovil_emisor = bancoDestino
            documentoInsert.banco_pagomovil_receptor = bancoOrigen
            documentoInsert.ip = ip
            documentoInsert.nro_telefono_pagomovil_emisor = objetoDeConsulta?.telefonoPagador
            documentoInsert.nro_telefono_pagomovil_receptor = objetoDeConsulta?.telefonoCobrador
            documentoInsert.documento_identidad_pagomovil_receptor = storeDocumento
            documentoInsert.documento_identidad_pagomovil_emisor = objetoDeConsulta?.id
