...
const InputTokenGenerator = ({
  ...
  const debouncedTargetFieldValue = useDebounce(modifiedData[attribute.targetField], 1000);    
  if(attribute.targetFieldArray){
    console.log(attribute.targetFieldArray);
    const variable = Array();
    attribute.targetFieldArray.forEach((Element,index) => {
       variable[index] = useDebounce(modifiedData[Element], 1000);
    });
    useEffect(() => {
      onChange({ target: { name, value: "", type: 'token' } });
  ...
  generateUid.current = async (shouldSetInitialValue = false) => {
    setIsLoading(true);
    const requestURL = getRequestUrl('token/generate');
    try {
      console.log(contentTypeUID, name);
      ...
      onChange({ target: { name, value: data, type: 'token' } }, shouldSetInitialValue);
      setIsLoading(false);
    } catch (err) {
      console.error({ err });
      setIsLoading(false);
...
InputTokenGenerator.defaultProps = {
  description: '',
  editable: false,
  error: null,
  validations: {},
...
export default InputTokenGenerator;
